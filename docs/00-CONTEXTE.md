...
| **L'audit technique : la synchronisation d'abord, pas de poids, pas de hors connexion, pas de matériel ancien** | Corrections de l'utilisateur le 11 septembre 2026 à la lecture du protocole de `08-RAPPORT-TEST.md` : « pas de limite 150 Ko, pas de mode avion, pas de D [matériel ancien]. Le plus important c'est que les annotations défilent bien et bien synchronisées avec le texte, c'est absolument essentiel ». Le protocole est réécrit dans cet ordre : la synchronisation dessin-texte, temps par temps de `02-CONTENU.md`, est le chantier A, avant tout le reste |

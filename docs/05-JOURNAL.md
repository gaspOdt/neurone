...
| **L'audit technique, vague 1 et début de la vague 2** | 11 et 12 septembre 2026. Des agents indépendants, un par chantier, chacun suivi d'un contre-vérificateur. Chantier A, la synchronisation : 42 constats, 38 confirmés. Chantiers B, F, G, H engagés. **Le défaut le plus grave** : en mouvement réduit, le bouton d'ouverture n'était branché par personne, donc le verrou de défilement ne se levait jamais et le site entier était fermé à ces visiteurs. Corrigé le 12 septembre, avec un contrôle qui n'emploie que des gestes. Rapport dans `08-RAPPORT-TEST.md` |
...
**Mesures du moment** : 33 contrôles sur 33 dans `outils-test-navigateur.py`,
...
Les huit pièces de la narration sont en ligne. Dans l'ordre :

1. **L'audit technique**, selon le protocole de `08-RAPPORT-TEST.md`.
2. **L'audit scientifique**, `09-AUDIT-SCIENTIFIQUE.md`, après le technique.
3. **Le parcours de démonstration**, `06-DEMO-3MIN.md`.

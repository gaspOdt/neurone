...
| **L'audit technique, mené** | 11 et 12 septembre 2026. Douze agents indépendants, un par chantier, chacun suivi d'un contre-vérificateur qui refaisait les gestes avant qu'un constat soit retenu. **86 constats rapportés, 73 confirmés** : 5 bloquants, 38 gênants, 30 cosmétiques ; 13 non reproduits. 86 lignes de protocole cochées, 51 non, 11 non mesurables ici. Rapport complet dans [`08-RAPPORT-TEST.md`](08-RAPPORT-TEST.md) |
| **Trois corrections pendant l'audit** | 12 septembre 2026. **En mouvement réduit, le site entier était fermé** : le bouton d'ouverture n'était branché par personne, donc le verrou de défilement ne se levait jamais (`3601190`). **Un piège au clavier** figeait le site à qui n'a pas de souris, quatre tabulations et Entrée depuis la page neuve (`1e5f111`). Et l'erreur `etatParcours`, cinquième occurrence du piège de la déclaration après le retour anticipé. Deux contrôles ajoutés, 13 et 14, qui n'emploient que des gestes réels |
| **Le signal afférent** | Pièce 8, 11 septembre 2026, demande de l'utilisateur. Huit petites flèches bleues, une par bout de dendrite, pointant vers l'intérieur, qui arrivent en glissant quand la caméra se pose sur les dendrites et repartent quand elle les quitte : le neurone reçoit. Calculées dans le générateur à partir des tracés des dendrites (`fleches_afferentes()`), montrées par `js/recit.js`. Sans script, elles sont là. Deux contrôles dans la batterie |
| **La chute de l'acte 2** | « Tout ça, pour un clic. Pas mal, non ? » Ajout de l'utilisateur, 11 septembre 2026 |
| **Les dessins réduits, deux fois** | Demande de l'utilisateur, deux fois le 11 septembre 2026 : le texte était dissimulé sous un dessin et deux rangs de boutons prenant les deux tiers de l'écran, puis « quand le texte défile et disparaît, on ne le voit pas assez ». Hauteur du dessin du parcours passée de 42 % à 31 % puis à **27 %** de l'écran (262 puis 228 px sur 844), boutons inchangés, zone de lecture de 266 à 359 puis **393 px**. Le dessin de l'ouverture plafonne à 300 px au lieu de 340, la courbe du corps cellulaire à 15 rem au lieu de 18 |
...
| 8. Le signal afférent sur les dendrites | **fait**, 11 septembre 2026 |
...
**Mesures du moment** : 37 contrôles sur 37 dans `outils-test-navigateur.py`,
mouvement réduit et sans JavaScript vérifiés par capture.

### Les deux audits de fin de projet

**1. Audit technique. FAIT**, les 11 et 12 septembre 2026 ; résultats dans
`08-RAPPORT-TEST.md`. Ce qui suit décrit ce qu'il couvrait.

Un agent parcourt le site et vérifie qu'il
...
Les huit pièces de la narration sont en ligne, et l'audit technique est
mené. Dans l'ordre :

1. **Décider du sort des trois constats bloquants qui restent ouverts**, tous
   trois demandant un arbitrage de l'utilisateur et non une simple
   correction : le zoom à 200 %, où la barre des boutons déborde de l'écran
   et la zone de lecture tombe à zéro ; le mouvement réduit, où le dessin
   n'est jamais à l'écran pendant qu'on lit le parcours ; et le retour au
   mouvement complet en cours de visite, qui rend la page illisible.
2. **Les 38 constats gênants**, par thème, en commençant par le décalage
   entre la phrase et la caméra, qui est la priorité déclarée de
   l'utilisateur.
3. **L'audit scientifique**, `09-AUDIT-SCIENTIFIQUE.md`.

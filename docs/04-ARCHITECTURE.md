...
| **Vérifier par un geste que le visiteur ne fait pas** | Le bouton d'ouverture n'était branché que sur le chemin animé : en mouvement réduit, le clic ne faisait rien et le verrou `overflow: hidden` ne se levait jamais. **Tout le site était fermé à ces visiteurs**, et trente contrôles n'y ont rien vu, parce qu'ils ouvrent la page avec `window.scrollTo`, qui ignore le verrou, et avec `.click()` de script, qui ne signale rien quand aucun gestionnaire n'écoute | Au moins un contrôle par mode doit n'employer que des GESTES : `Input.dispatchMouseEvent` à la position du bouton, puis `nav.molette()`. C'est le contrôle 13 |
| **Déclaration après le retour anticipé, cinquième fois** | `etatParcours`, déclaré près de la fonction qui l'écrit, donc après le retour anticipé du mouvement réduit que `majCamera()` emprunte : `ReferenceError` sur le seul bloc porteur d'un `data-cadrage`, et le gros plan sur la synapse n'avait jamais lieu. Invisible ailleurs parce que `.filter()` sur un tableau vide n'appelle jamais son rappel | La règle ne souffre aucune exception, et elle se paie une fois de plus à chaque nouvel état. Tout l'état d'`initRecit` se déclare avant le premier `return` |
...
| **Un `z-index` se compte dans l'empilement de son parent** | Un vrai clic au centre du bouton « Clique » ne faisait rien : le cache plein écran a beau être en `z-index: 5`, il est DANS la pile du haut, empilement de niveau 1, et la pile du bas, même niveau et plus loin dans le document, passait devant tout le cache. Invisible, ses textes étant à opacité nulle, mais elle prenait les clics. Latent depuis le début sur la moitié basse du bouton, déclenché le 14 septembre 2026 par un titre plus long d'une ligne | Tester la zone cliquable par `elementFromPoint` sur toute la surface d'un bouton, pas seulement en son centre, et à plusieurs tailles d'écran. Un élément censé couvrir la page doit être remonté avec son **parent** qui porte l'empilement |
| **Espace ordinaire devant `: ; ? !`** | Sur téléphone, une ligne commençait par « : chacun ». Invisible à toute mesure, vu sur capture | `insecables()` dans `outils-dessin-neurone.py` pose l'espace insécable à la génération, sur tout le texte du récit. Ne pas écrire `&nbsp;` à la main dans les phrases, ne pas retirer l'appel |
